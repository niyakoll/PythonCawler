from lihkg_scan import scan as startScanning
import sendWhatsapp
import lihkg_result_text_cleaning
import ai_agent
import json
import threading
import time
...
SHARED_DIR = os.path.abspath(SHARED_DIR)  # 轉成絕對路徑

if SHARED_DIR not in sys.path:
    sys.path.insert(0, SHARED_DIR)  # insert(0, ...) 優先級最高

# 現在安全 import
try:
    from shared_pidlock import acquire_lock, release_lock
    print(f"成功載入 shared_pidlock from: {SHARED_DIR}")
except ImportError as e:
    print(f"載入 shared_pidlock 失敗！請確認 shared 資料夾存在：{SHARED_DIR}")
    raise e
#import pidlock
import pytz
...
def Distribute(clientName,targetWhatsappGroup,whapi_group_id):
    ...
    try:
        cleanResult += lihkg_result_text_cleaning.formatText("lihkg_result",clientName)
...
def combineList():
    ...
    with open(MANIFEST_PATH, 'r', encoding="utf-8") as f:
        ...
        for client in clientList:
            if client_panel[client]["lihkg_run"] == False:
                continue
...
def clearSearchResultJson():
    ...
    try:
        for i in range(1,6):
            with open(str(os.path.join(os.path.dirname(__file__),"result",f"lihkg_result{i}.json")), 'w',encoding="utf-8") as file:
...
def quietMode_off(client,client_message_interval,whapi_group_id):
    ...
    try:
        match client_message_interval :
            case 15:
                aiText, recentPostList = lihkg_result_text_cleaning.prepareOutputText(client,client_message_interval)
