        ...
        with open(str(os.path.join(os.path.dirname(__file__),"result",f"{resultFileName}{r}.json")), 'r',encoding="utf-8") as file:
            ...
            if result:
                ...
                for post_item in clientResult:
                    whole_post_content = post_item["data"]["response"]["item_data"]
                    post = whole_post_content[0]#the first data is the post content
                    postSource = post["source"]
                    postKeyword = post["keyword"]
                    #print(f"postKeyword:{postKeyword}")
                    postlink = post["link"]
                    postTitle = post["title"]
                    postid = post["postid"]
                    postReleaseDate = post["releaseDate"]
                    postUpdateTime = post["updateTime"]
                    ...
                    try:
                        ...
                        for comment in reply:
                            ...
                            commentTime = comment["releaseDate"]
                            commentUpdateTime = comment["updateTime"]
                            ...
                            commentObject = {
                                ...
                                "text":commentText,
                                "commentTime":timestampConvert(commentTime),
                    ...
                    try:
                        if len(outputRecord) == 0: #do not contain any record
                            ...
                            if postlink in recordUrl: #check if the post url is already in record
                                if (int(postLikeCount) <= int(outputRecord[postlink]["postLikeCount"])) and  (int(postCommentCount) <= int(outputRecord[postlink]["postReplyCount"])):
...
def getGobalattrFromManifest(attr:str):
    ...
    #test_get_result("lihkg_result","特朗普")
    #AiinputText, postListMessage = prepareOutputText("特朗普",15)
    #print(postListMessage)