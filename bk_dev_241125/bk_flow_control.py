from bk_scan import scan as startScanning
import sendWhatsapp
import bk_result_text_cleaning
import ai_agent
import json
import threading
import time
...
def Distribute(clientName,targetWhatsappGroup,whapi_group_id):
    ...
    try:
        cleanResult += bk_result_text_cleaning.formatText("bk_result",clientName)
...
def combineList():
    ...
    with open(MANIFEST_PATH, 'r', encoding="utf-8") as f:
        ...
        for client in clientList:
            if client_panel[client]["bk_run"] == False:
                continue
...
def clearSearchResultJson():
    ...
    try:
        for i in range(1,6):
            with open(str(os.path.join(os.path.dirname(__file__),"result",f"bk_result{i}.json")), 'w',encoding="utf-8") as file:
...
def quietMode_off(client,client_message_interval,whapi_group_id):
    ...
    try:
        match client_message_interval :
            case 15:
                aiText, recentPostList = bk_result_text_cleaning.prepareOutputText(client,client_message_interval)
